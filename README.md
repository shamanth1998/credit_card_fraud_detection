...
Detect Fraudulent Credit Card transactions using tree based Machine Learning models and deep learning model and compare performances

In this notebook, I explore various Machine Learning models to detect fraudulent use of Credit cards. I compare each model performance and results. The best performance is achieved using undersampling technique.
...
Note that prevalence of fraudulent transactions is very low in the dataset. Less than 0.1% of the card transactions are fraudulent. This means that a system predicting each transaction to be normal can reach an accuracy of over 99.9% despite not detecting any fraudulent transaction.so even if we get an accuracy of 99.9%  its not a good model so we have to balance
...
The best results are achieved by under sampling technique.
